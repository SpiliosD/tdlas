...
class scpi(object):
    ...
    def gen_set(
        ...
        """
        self._validate_gen_set_params(chan, func, volt, freq, offset, phase, dcyc, data, trig_sour, ext_trig_deb_us,
                                      ext_trig_lev, load, sdrlab, siglab)
    ...
    def _validate_gen_set_params(
        ...
        assert not (
                    siglab and sdrlab), "Please select only one board option. 'siglab' and 'sdrlab' cannot be true at the same time."
    ...
    def _validate_acq_set_params(
        ...
        assert (dec not in dec_fact_list) and (
                    16 <= dec <= 65536), "Decimation factor out of range [1,2,4,8,16,17,18,...,65536]"
        if units is not None:
    ...
    def _validate_acq_trig_params(
        ...
        """
        trig_lvl_lim = 20.0 if any(
            self.txrx_txt(f"ACQ:SOUR{i + 1}:GAIN?").upper() == "HV" for i in range(4 if input4 else 2)) else 1.0
        ...
        if siglab and ext_trig_lvl is not None:
            assert abs(
                ext_trig_lvl) <= ext_trig_lvl_limit, f"External trigger level out of range {-ext_trig_lvl_limit, ext_trig_lvl_limit} V"
        if ext_trig_deb_us is not None:
            assert ext_trig_deb_us >= 1, "External trigger debounce filter value is out of range. The minimal value is 1 microsecond"
        assert not (
                    siglab and input4), "Please select only one board option. 'siglab' and 'input4' cannot be true at the same time."
        self._validate_board(siglab, input4)
    ...
    def _validate_acq_split_trig_params(
        ...
        assert abs(
            trig_lvl) <= trig_lvl_lim, f"Trigger level out of range {-trig_lvl_lim, trig_lvl_lim} V for gain {gain_lvl}"
        assert trig_delay >= 0, "Trigger delay cannot be less than 0"
    ...
    def _validate_uart_params(
        ...
        """
        speed_list = [1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400, 576000, 921000, 1000000, 1152000,
    ...
    def spi_set(
        ...
        try:
            ...
            raise ValueError(
                f"Word length must be greater than {bits_min_limit}. Current word length: {word_len}") from bits_err
